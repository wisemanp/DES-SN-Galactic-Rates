...
%DIF DEL ../../Writing/draft.tex                         Mon Mar  8 17:19:32 2021
%DIF ADD ../../Writing/DES-SN-Galactic-Rates/draft.tex   Wed Mar 10 18:26:12 2021
%
...
\DIFdelbegin \DIFdel{We introduce the SN host and field galaxy samples in Section \ref{sec:data} and describe our }\DIFdelend \DIFaddbegin \DIFadd{In Section \ref{sec:data} we introduce our large sample of SNe Ia from the Dark Energy Survey as well as our deep sample of field galaxies. We describe our detailed }\DIFaddend handling of incompleteness in Section \ref{sec:incompleteness}. We present the SN Ia rate per galaxy in Section \ref{sec:rates}. We outline \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{our }\DIFaddend modelling of star-formation histories and \DIFaddbegin \DIFadd{our novel }\DIFaddend measurement of the \DIFaddbegin \DIFadd{power-law }\DIFaddend DTD in Section \ref{sec:model}. In Section \ref{sec:split_x1_c} we investigate how the DTD differs among sub-populations of SNe with different light curve characteristics\DIFaddbegin \DIFadd{, in particular the stretch, and show stretch to be strongly dependent on progenitor age}\DIFaddend . We conclude in Section \ref{sec:conclusion} \DIFaddbegin \DIFadd{by discussing the implications of the result}\DIFaddend .
...
\DIFaddbegin \DIFadd{Finally, at time $t$, some time $\tau$ since an epoch of star formation, a fraction of stellar mass is lost. We adopt the parametrization used by \mbox{%DIFAUXCMD
...
\DIFaddend As per \citet{Childress2014} we plant seed galaxies with initial masses of \DIFdelbegin \DIFdel{$10^6 \mathrm{M}_{\odot}$ }\DIFdelend \DIFaddbegin \DIFadd{$10^6 ~\mathrm{M}_{\odot}$ }\DIFaddend at intervals of 50 Myr for look-back times $1 \leq t_f \leq 10$ Gyr, and 25 Myr for look-back times $10 \leq t_f \leq 13$ Gyr \DIFdelbegin \DIFdel{, and evaluate the model with a time step }\DIFdelend \DIFaddbegin \DIFadd{to achieved a more detailed model at early times with high SFR. For each seed galaxy, we evaluate Eq. \ref{eq:sfh_model} at time steps }\DIFaddend of 0.5 Myr \DIFdelbegin \DIFdel{.
...
\begin{figure}
    ...
    \caption{As per Fig. \ref{fig:rate_fitted} but with the data split by their stretch parameter $x_1$. \DIFaddbeginFL \DIFaddFL{Fits were performed with the DTD slope $\beta$ and normalisation $A$ as free parameters, and prompt time $t_{\mathrm{p}}$ fixed at 0.04 Gyr.}\DIFaddendFL %The blue lines indicate the best fitting model based on a $t^{\beta}$ power law DTD convolved with SFHs based on the \citet{Childress2014} model}
