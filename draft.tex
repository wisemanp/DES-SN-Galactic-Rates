...
\begin{abstract}
Measuring the rate of type Ia supernovae (SNe Ia) is crucial method by which to understand the progenitors and physical processes involved in these explosions. Since SNe Ia are a key source of heavy elements, the SN Ia rate is also an important ingredient in models of galaxy evolution. We use a sample of 1411 photometrically classified SNe discovered by the Dark Energy Survey (DES) along with \textbf{final number} field galaxies to calculate the rate of SNe Ia per galaxy as a function of galaxy stellar mass across a broad range of scales $8.5 \leq \log(M_*/\mathrm{M}_{\odot}) \leq 11.25$. We find that the SN Ia rate increases with stellar mass with a slope of $0.70 \pm 0.02$, which is consistent with previous work. We use an empirical model of stellar mass assembly to estimate the average star-formation histories (SFHs) of galaxies across the stellar mass range of our measurement. We combine the modelled SFHs with the SN Ia rates to estimate constraints on the SN Ia delay time distribution (DTD), and find a power-law with index $\beta = -0.96 \pm 0.5$ and normalisation $A = 1.7 \pm0.05 \times 10^{-13}~\mathrm{SNe}~{\mathrm{M}_{\odot}}^{-1}~\mathrm{yr}^{-1}$. 
\end{abstract}
...
The numbers of SN hosts and field galaxies passing these cuts are listed in Table \ref{tab:cuts}. The final samples comprise 1204 SN host galaxies and 48,177 field galaxies.
...
\textbf{Or's suggestion: Fix beta, A and fit for tp}
